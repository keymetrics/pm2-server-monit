  ...
  shelljs.exec('ifconfig', { async : true, silent : true }, function(err, out) {
    ...
    blocks.forEach(function (block) {
      ...
      if (block.device != 'lo' && block.other.rxBytes != 0 && block.other.txBytes != 0) {
        ...
        if (old) {
          metrics[block.device].input.set(((block.other.rxBytes - old.other.rxBytes) / 1000000).toFixed(2) + ' MB/s')
          metrics[block.device].output.set(((block.other.txBytes - old.other.txBytes) / 1000000).toFixed(2) + ' MB/s')
    ...
    //set total
    metrics.total.input.set((totalRx / 1000000).toFixed(2) + ' MB/s')
    metrics.total.output.set((totalTx / 1000000).toFixed(2) + ' MB/s')
    oldStats = newStats
  })
}
