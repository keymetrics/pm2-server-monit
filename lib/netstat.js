...
function refreshNet() {
  shelljs.exec('ip -s a', { async : true, silent : true }, function(err, out) {
    if (err)
    ...
    for (var i = 0; i < stats.length; i++) {
      ...
      if (metrics[stats[i].interface]) {
		if (oldStats && oldStats[i]) {
          var output = (stats[i].outputBytes - oldStats[i].outputBytes) /  125000;
          var input = (stats[i].inputBytes - oldStats[i].inputBytes) / 125000;
        }
        else {
          var output = 0
          var input = 0;
