  ...
  setInterval(function() {
    if (os.platform() == 'darwin') {
      ...
      ifaces.forEach(function (iface) {
        if (iface.indexOf('en') != -1) {
          var res = cp.execSync('netstat -wl -I ' + iface).toString()
          res = res.split('\n')
          var val = res[1].split(' ')
          var rx = parseInt(val[4])
          var tx = parseInt(val[6])
          rxTotal += rx;
          txTotal += rx;
          if (!oldStats[iface]) {
            oldStats[iface] = {
          ...
          setInterface(iface, oldStats[iface], {tx: tx, rx: rx})
        }
      })
      setInterface('total', oldStats['total'], {tx: totalTx, rx: totalRx})
      return;
    }
